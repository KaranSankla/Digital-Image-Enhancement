...
def load_model_pc():
    ...
    return pc
print(f"Directory path: {__READPATH__}")
print(f"Files in directory: {os.listdir(__READPATH__)}")
for f in filename:
    print(f"Processing file: {f}.txt")
    print(f"File exists: {os.path.exists(__READPATH__ + f + '.txt')}")
    print(f"Content:\n{np.loadtxt(__READPATH__ + f + '.txt')}")
